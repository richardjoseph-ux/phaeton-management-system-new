 ...
 const filtered = useMemo(() => {
  ...
  const exportPDF = () => {
    ...
    filtered.forEach(trip => {
      ...
      const row = [trip.plate_number || '', (trip.owner_name || '').substring(0, 12), trip.truck_type || '', (trip.client_name || '').substring(0, 14), (trip.pickup_location || '').substring(0, 14), (trip.delivery_location || '').substring(0, 14), trip.delivery_code || '', trip.delivery_date || '', trip.dr_number || '', trip.waybill_number || '', (trip.particular || '').substring(0, 12), (trip.billing_cycle_name || '').substring(0, 14), trip.gross_rate ? `P${trip.gross_rate.toFixed(0)}` : '', trip.net_payroll ? `P${trip.net_payroll.toFixed(0)}` : ''];
      ...
      <div className="bg-card border rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          ...
          <Select value={filters.date_mode} onValueChange={v => setF('date_mode', v)}>
            ...
            <SelectContent>
              <SelectItem value="delivery">By Delivery Date</SelectItem>
              <SelectItem value="cheque">By Cheque Date</SelectItem>
            </SelectContent>
          </Select>
          <Select value={filters.year} onValueChange={v => setF('year', v)}><SelectTrigger><SelectValue placeholder="All Years" /></SelectTrigger><SelectContent><SelectItem value="all">All Years</SelectItem>{availableYears.map(y => <SelectItem key={y} value={y}>{y}</SelectItem>)}</SelectContent></Select>
          <Select value={filters.client_id} onValueChange={v => setF('client_id', v)}><SelectTrigger><SelectValue placeholder="All Clients" /></SelectTrigger><SelectContent><SelectItem value="all">All Clients</SelectItem>{clients.map(c => <SelectItem key={c.id} value={c.id}>{c.client_name}</SelectItem>)}</SelectContent></Select>
          <Select value={filters.sub_id} onValueChange={v => setF('sub_id', v)}><SelectTrigger><SelectValue placeholder="All Subcontractors" /></SelectTrigger><SelectContent><SelectItem value="all">All Subcontractors</SelectItem>{subcontractors.map(s => <SelectItem key={s.id} value={s.id}>{s.owner_name} – {s.plate_number}</SelectItem>)}</SelectContent></Select>
          <Select value={filters.cycle_id} onValueChange={v => setF('cycle_id', v)}><SelectTrigger><SelectValue placeholder="All Billing Cycles" /></SelectTrigger><SelectContent><SelectItem value="all">All Billing Cycles</SelectItem>{billingCycles.map(b => <SelectItem key={b.id} value={b.id}>{b.cycle_name}</SelectItem>)}</SelectContent></Select>
        ...
        <div className="bg-card border rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">{['Plate #', 'Owner', 'Truck', 'Client', 'Pickup → Delivery', 'Del. Date', 'DR #', 'Waybill', 'Particular', 'Billing Cycle', 'Cheque Date', 'Gross', 'Net'].map(h => (<th key={h} className="text-left px-3 py-3 font-semibold text-xs text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>))}</tr>
              </thead>
              <tbody>
                {paginatedData.length === 0 ? (
                  <tr><td colSpan={13} className="text-center py-16"><BarChart3 className="w-10 h-10 text-muted-foreground/30 mx-auto mb-2" /><p className="text-muted-foreground text-sm">No records match the selected filters</p></td></tr>
                  ...
                  <tr key={trip.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-xs"><div>{trip.pickup_location}</div><div className="text-muted-foreground">→ {trip.delivery_location} <span className="font-mono">({trip.delivery_code})</span></div></td>
                    ...
                    <td className="px-3 py-2.5 text-xs">{trip.first_cheque_date ? formatDateDisplay(trip.first_cheque_date) : <span className="text-gray-400 italic">No Date</span>}</td>
