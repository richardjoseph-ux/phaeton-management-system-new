...
export default function BillingCycles() {
  ...
  const [summaryClientFilter, setSummaryClientFilter] = useState('all');
...
const syncClientIds = async () => {
  ...
  useEffect(() => {
    if (mainTab === 'summary' && !tripsLoaded) {
      base44.entities.TripRecord.list('-created_date', 500).then(t => {
        setAllTrips(t);
        setTripsLoaded(true);
      });
    }
...
const billingReceivedGroups = (() => {
  ...
  const getSummaryRecord = (date, clientId) => displaySummaryRecords.find(
    r => r.billing_received_date === date && String(r.client_account_id || '') === String(clientId || '')
  );
...
const getChequeAmountForDate = (date, clientId) => {
  const cyclesForDate = cycles.filter(c => c.billing_received_date === date && String(c.client_account_id || '') === String(clientId || ''));
  ...
  // Scope deductions by the plates that ran trips for this client on this date.
  const plates = new Set(relevantTrips.map(t => t.plate_number).filter(Boolean));
  const relevantDeductions = deductions.filter(
    d => d.billing_received_date === date && (!d.plate_number || plates.has(d.plate_number))
  );
...
const archivedSummaryGroups = billingReceivedGroups
  .filter(g => getSummaryRecord(g.date, g.client_account_id)?.is_archived)
  .map(g => ({ ...g, cycles: [...g.cycles].sort((a, b) => a.cycle_name.localeCompare(b.cycle_name)) }))
  .sort((a, b) => b.date.localeCompare(a.date) || String(a.client_account_id || '').localeCompare(String(b.client_account_id || '')));
  ...
  useEffect(() => { setSummaryPage(1); setSummaryClientFilter('all'); }, [summaryTab]);
  useEffect(() => { setSummaryPage(1); }, [summaryClientFilter]);
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {loading ? (
        ...
        <>
          ...
          {(() => {
            const rawGroups = summaryTab === 'active' ? activeSummaryGroups : archivedSummaryGroups;

            const summaryClientOptions = [...new Set(
              rawGroups.flatMap(g => g.cycles.map(c => getClientName(c.client_account_id))).filter(n => n && n !== '—')
            )].sort();
            ...
            return (
              <>
                <div className="mb-4">
                  <Select value={summaryClientFilter} onValueChange={setSummaryClientFilter}>
                    <SelectTrigger className="w-56 h-8 text-sm">
                      <SelectValue placeholder="All Clients" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">All Clients</SelectItem>
                      {summaryClientOptions.map(name => (
                        <SelectItem key={name} value={name}>{name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                {groups.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-muted-foreground text-sm">
                      {summaryClientFilter !== 'all' ? 'No records for selected client' : summaryTab === 'active' ? 'No billing received dates recorded yet' : 'No archived summaries'}
                    </p>
                  </div>
                ) : (
                  <div className="bg-card border rounded-lg overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-muted/50">
                          {['Billing Received Date', 'Client', 'Payout Date', 'Statements', '# Stmts', 'Cheque Amount', 'Paid', 'Payroll Processed', ''].map(h => (
                            <th key={h} className="text-left px-4 py-3 font-semibold text-xs text-muted-foreground uppercase tracking-wide">{h}</th>
                          ))}
                        </tr>
                      </thead>
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="border-t bg-muted/50">
                      ...
                      <td className="px-3 py-3 text-right font-bold whitespace-nowrap">₱{trips.reduce((s, t) => s + (t.gross_rate || 0) - (t.tax_deduction || (t.gross_rate || 0) * 0.02), 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
