...
export default function BillingReceivedSummaryDialog({ open, onClose, billingDate, cycles, fuelSubsidies }) {
  ...
  // Revenue adjustments (Other Charges loaded for this billing date)
  ...
  const totalDeductions = billingDeductions.reduce((sum, d) => sum + (d.insurance_charge || 0) + (d.other_charges || 0), 0);

  const finalGrandTotalGross = grandTotals.gross + totalOtherCharges;
  const totalTax = grandTotals.tax + taxOnOtherCharges;
