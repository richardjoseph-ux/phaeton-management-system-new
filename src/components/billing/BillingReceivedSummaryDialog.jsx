...
export default function BillingReceivedSummaryDialog({ open, onClose, billingDate, cycles, fuelSubsidies }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const [allTrips, deductions, reimbs, other] = await Promise.all([
      ...
      base44.entities.OtherCharge.filter({ billing_received_date: billingDate }, 'charge_type', 200), // Fetch added
    ]);
    setTrips(allTrips);
    setBillingDeductions(deductions);
    setReimbursements(reimbs);
    setOtherCharges(other); // Set state
    setLoading(false);
  };
  ...
  const plateGroups = (() => {
    ...
    return Object.values(groups).map(row => {
      ...
      return { 
        ...
        net: row.tripNet - insurance - other + totalReimbursement + (totalOtherCharges / Object.keys(groups).length) 
      };
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-5">
                ...
                <p className="text-2xl font-bold mt-2 text-amber-700">₱{chequeAmount.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
