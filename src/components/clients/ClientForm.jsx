...
export default function ClientForm({ open, onClose, onSaved, editData, isAdmin = false }) {
  ...
  const setRoute = (idx, k, v) => isAdmin && setForm(p => {
    const routes = [...p.routes];
    routes[idx] = { ...routes[idx], [k]: v };
    return { ...p, routes };
  });
  const setRouteRate = (idx, truckType, v) => isAdmin && setForm(p => {
    const routes = [...p.routes];
        ...
        <div className="overflow-y-auto flex-1 space-y-5 py-2 pr-1">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-muted/20 border-x border-border px-3 py-2">
              ...
              <input type="text" value={routeSearch} onChange={e => setRouteSearch(e.target.value)} placeholder="Search by destination or route code..." className="flex-1 text-xs bg-transparent outline-none placeholder:text-muted-foreground" />
              {routeSearch && <button type="button" onClick={() => setRouteSearch('')} className="text-muted-foreground hover:text-foreground"><X className="w-3.5 h-3.5" /></button>}
            </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-muted/60 border-b">
                    {activePickup === '__all__' && <th className="text-left px-3 py-2.5 text-xs font-semibold text-muted-foreground border-r">Pick Up</th>}
                    ...
                    {rateColumns.map(t => <th key={t} className={`text-center px-3 py-2.5 text-xs font-semibold border-r ${activeTruck === t ? 'text-white bg-primary' : 'text-primary'}`}>{t}</th>)}
